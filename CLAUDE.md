...
品質向上のために以下の3エージェントを運用する。
各エージェントは Claude Code のセッション内で必要に応じて呼び出す。

### エージェント一覧

| エージェント | 主な役割 | 起動タイミング |
|---|---|---|
| QA エージェント | プロジェクト固有ルールの整合性検証 | コミット前・大きな変更後 |
| デザイン＆UX エージェント | Deep Aurora 一貫性・レスポンシブ・アクセシビリティ | CSS / HTML 変更後 |
| リサーチエージェント | 外部情報の調査とコンテンツ反映先の提案 | コンテンツ更新の検討時 |
...
**担当範囲：**
- HTML の壊れたリンク・`id` 参照の整合性チェック
- `assets/base.css` に存在しない CSS 変数を参照していないかの検出
- `mitre/js/xlink.js` の `postMessage` フォーマットと受信側（map / scenes / topology）の整合性確認
- JS のコンソールエラー・未定義変数の静的チェック
- 新ページ追加時の `favicon/head-snippet.html` 適用漏れ確認

**呼び出し例（Claude Code セッション内）：**
```
QAエージェントとして動いてください。
今回変更した [ファイル名] に対して、CLAUDE.md の QA エージェント担当範囲を
すべてチェックして問題点を報告してください。ファイルは編集しないでください。
```

**既存スキルとの使い分け：**
- `/code-review` → スポットでのバグ・改善点レビュー（都度）
- `/security-review` → 機密情報漏れ等のセキュリティ監査
- QA エージェント → このプロジェクト固有ルール（CSS 変数整合・xlink ペア等）の継続的チェック
...
**担当範囲：**
- `assets/base.css` のトークン変数が正しく参照されているか（ハードコードされた色・影・フォントの検出）
- `assets/responsive.css` の意図（`:has(.dtabs)` ロジック等）に反するスタイルが混入していないか
- モバイル幅（375px・768px）での表示崩れの指摘
- 新コース追加時に既存3コース（aaism / ccaf / cissp）と HTML 構造・CSS 構成が揃っているかの確認
- アクセシビリティ（コントラスト比・ARIA 属性・キーボードナビゲーション）

**呼び出し例（Claude Code セッション内）：**
```
デザイン＆UXエージェントとして動いてください。
今回変更した [ファイル名] が CLAUDE.md のデザイン＆UX エージェント担当範囲を
満たしているかチェックして問題点を報告してください。ファイルは編集しないでください。
```
...
**担当範囲：**
- MITRE ATT&CK のバージョンアップ差分の調査（現在 v19）
- CISSP CBK・AAISM・CCA-F の試験内容変更・新情報の調査
- 新しいセキュリティトピック・攻撃手法のコンテンツ案提案
- 調査結果を「どのファイルのどのセクションに反映すべきか」まで整理して返す

**制約：ファイルの編集は行わない。調査と反映先の提案のみ。**

**呼び出し例（Claude Code セッション内）：**
```
リサーチエージェントとして動いてください。
[調査テーマ] について調査し、このリポジトリのどのファイル・セクションに
反映すべきかを提案してください。ファイルは編集しないでください。
```
...
3. デザイン＆UX エージェント
   └─ 変更後の CSS/HTML がデザインシステムに適合しているか確認

4. QA エージェント
   └─ リンク・JS・CSS 変数の整合性を検証 → 問題なければコミット・push
```
