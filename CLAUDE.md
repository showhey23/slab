...
**slab は「公開サイト専用」リポジトリ。完成した HTML のみを受け取る。**

| リポジトリ | 役割 |
|---|---|
| `slab-data` | 入力データ管理（CSV・JSON） |
| `slab-workshop` | コンテンツ制作・検証（HTML 生成） |
| **`slab`（このリポジトリ）** | **公開サイト（GitHub Pages）** |

制作フロー：`slab-data` → `slab-workshop` → **`slab`**（ここ）→ 公開
...
**制作・検証は `slab-workshop` で完結させること。`slab` では最終確認のみ行う。**

### 統合リグレッションエージェント（このリポジトリ専用）

`slab-workshop` から完成 HTML を受け取った後、公開前の最終ゲートとして実行する。

**担当範囲：**
- 全コース（aaism / ccaf / cissp / mitre）のトップページが正常に表示されるか
- `index.html` のポータルカードリンクが全コースに正しく張られているか
- 相対パス整合性（`../index.html` で親に戻れるか）
- 各ページの `<head>` に `favicon/head-snippet.html` 相当の記述があるか
- `mitre/js/xlink.js` の postMessage フォーマットと受信側の整合性

**呼び出し例：**
```
統合リグレッションエージェントとして動いてください。
slab-workshop から受け取った最新コンテンツに対して、CLAUDE.md の
統合リグレッションエージェント担当範囲をすべてチェックして問題点を報告してください。
ファイルは編集しないでください。
```

**既存スキルとの使い分け：**
- `/code-review` → スポットでのバグ・改善点レビュー
- `/security-review` → 機密情報漏れ等のセキュリティ監査
- 統合リグレッションエージェント → 公開前の全体整合性確認（このリポジトリ固有）

> QA・デザイン・リサーチ・構造化の各エージェントは `slab-workshop` の CLAUDE.md を参照。
