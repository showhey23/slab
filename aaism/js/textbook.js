...
(function(){
  ...
  function onScroll(){
    ...
    try{localStorage.setItem('aaism-scroll-'+dom.dataset.domain,window.scrollY);}catch(e){}
  }
  function activate(n,restore){
    ...
    try{localStorage.setItem('aaism-domain',n);}catch(e){}
    var y=0;if(restore){try{y=+localStorage.getItem('aaism-scroll-'+n)||0;}catch(e){}}
  ...
  var start=1;try{start=+localStorage.getItem('aaism-domain')||1;}catch(e){}
  var hm=(location.hash||'').match(/^#d(\d)-/);if(hm)start=+hm[1];
  if(start<1||start>4)start=1;
  activate(start,true);
})();
